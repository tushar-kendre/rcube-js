...
import {
  getPiecesInFaceLayer,
  getPiecesInLayer,
  rotatePosition,
  rotateStickerFace,
} from "./cube-piece-utils";
...
export function parseMove(notation: MoveNotation): Move {
  ...
  const layer = layerStr
    ? Math.max(1, Math.min(parseInt(layerStr, 10), Infinity))
    : 1;
  ...
  layerPieces.forEach((piece) => {
    ...
    if (targetPiece) {
      ...
      const rotatedStickers = targetPiece.stickers.map((sticker) => {
        const newFace = rotateStickerFace(
          sticker.face,
          move.face,
          move.clockwise,
        );
      ...
      if (piece.type === "center" && rotatedStickers.length === 1) {
        ...
        (targetPiece as EdgePiece).stickers = [
          rotatedStickers[0],
          rotatedStickers[1],
        ];
      } else if (piece.type === "corner" && rotatedStickers.length === 3) {
        (targetPiece as CornerPiece).stickers = [
          rotatedStickers[0],
          rotatedStickers[1],
          rotatedStickers[2],
        ];
